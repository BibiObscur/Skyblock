...
public class Plugin extends JavaPlugin{
	...
	public void onEnable() {
        ...
        try {
        	...
        	if(new File(directory + "orphanedIslands.bin").exists()) {
        		...
        		System.out.println("[BibiSkyblock] Chargement des iles supprimées.");
				if(null != load)
					orphaned = load;
			}else
	...
	public void defineLevel() {
    	...
    	for(Entry<String, Island> entry : playerIslands.entrySet()) {
	    			...
	    			for(k = island.z - ISLAND_SPACING/2; k < island.z + ISLAND_SPACING/2; k++) {
	        			...
	        			if(block.getType() != Material.AIR && block.getType() != Material.WATER) {
	        				if(block.getType() == Material.GRASS || block.getType() == Material.MYCEL)
	        					xpblock[1] += 1 - ((xpblock[1] > 576)?0.5:0);
    		...
    		xp = 1 * xpblock[0] + 8 * xpblock[1] + 6 * xpblock[2] + 5 * xpblock[3] + 2 * xpblock[4] + 4 * xpblock[5] + 2 * xpblock[6] + 4 * xpblock[7] + 0.5 * xpblock[8] + 500 * island.challenges.size();
	...
	public void addPlayerToGroup(String leader, String playername) {
		if(!hasGroup(playername)) {
			...
			if(isConnected(leader)) getServer().getPlayer(leader).sendMessage(ChatColor.GREEN + "Le joueur " + ChatColor.BLUE + playername + ChatColor.GREEN + " a été ajouté à votre groupe.");
	...
	public void removePlayerFromGroup(String leader, String playername) {
		if(hasGroup(leader) && leader != playername) {
			...
			if(group.getMembers().isEmpty())
				...
				group.getMembers().add(leader);
			if(isConnected(leader)) getServer().getPlayer(leader).sendMessage(ChatColor.RED + "Le joueur " + ChatColor.BLUE + playername + ChatColor.RED + " a été exclu de votre groupe.");
			if(isConnected(playername)) getServer().getPlayer(playername).sendMessage(ChatColor.RED + "Vous avez été exclu du groupe par " + ChatColor.BLUE + leader + ChatColor.RED + ".");
		}
	}
	...
	public void quitGroup(String playername) {
		if(hasGroup(playername) && !isLeader(playername)) {
			...
			playerHomes.remove(playername);
			if(isConnected(playername)) getServer().getPlayer(playername).sendMessage(ChatColor.RED + "Vous avez quitté le groupe de " + ChatColor.BLUE + group.getLeader() + ChatColor.RED + ".");
		}
	}
	...
	public void removeGroup(String leader) {
		if(hasGroup(leader)) {
			...
			if(isConnected(leader)) getServer().getPlayer(leader).sendMessage(ChatColor.RED + "Votre groupe a été dissout");
		}
	}
	...
	public void changeLeader(String oldleader, String newleader) {
		if(hasGroup(oldleader)) {
			...
			if(isConnected(newleader)) getServer().getPlayer(newleader).sendMessage(ChatColor.GREEN + "Vous êtes le nouveau leader du groupe !");
		}
	}
